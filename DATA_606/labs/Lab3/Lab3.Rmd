...
- A streak of 1 means one H followed by a M, there are 1 H and 1 M in a streak of 1. A streak of 0 is just a M no H.
...
```{r}
kobe_streak %>% group_by(length) %>% summarise(n())
```
 
 - The graph is right sewed, his typical streak length was between 0 and 1, making up ~83% of his throws. His longest streak was 39 of length 0.
...
```{r set-seed}
set.seed(42)                 # make sure to change the seed
```

```{r}
fair_coin <- sample(c('head', 'tail'), size=100, replace=T)
unfair_coin <- sample(c('head', 'tail'), size=100, replace=T, prob=c(0.2,0.8))
table(fair_coin)
table(unfair_coin)
```
- The unfair coin came up heads 17 times which is approaching the true value of 20%
...
- With a higher M probability the count of 0 streak is the largest (65/133~=49%). Streaks >= 1 accounts for the other 50% of the data.
...
- Rerunning the same code with the same seed will produce exactly the same result. 

```{r}
calc_streak(sim_basket) %>% ggplot(aes(x = length)) + geom_bar()
```
...
```{r}
calc_streak(sample(shot_outcomes, size = 133, replace = TRUE, prob=c(0.60, 0.40))) %>% ggplot(aes(x = length)) + geom_bar()
```

- When looking at a simulated shooter who has hot hands we see very long streaks, up to a length of 8, with even small streaks happening more than 10 times

Kobe distribution

```{r}
calc_streak(kobe_basket$shot) %>% ggplot(aes(x = length)) + geom_bar()
```

- Kobe distribution doesn't have long streaks, the longest streak being 4 which occurred only once. Most of his streaks is 0,1,2 which any amateur player (me) could do.
