...
As the binwidth get's bigger less information is shown in the histogram. The third plot binwidth=150 completely eclipses the second plot binwidth=15 thus hidding the fact that most delays were closer to 0 didn't have an abrupt drop. Furthermore as the binwidth get's too small we are seeing more data than we need, binwidth=15 is telling the same story as binwidth=30. A lot of 0 minute delays and a slow increase to 250 minutes.
...
1.  Create a new data frame that includes flights headed to SFO in February, 
    ...
    32 Unique flights meet the criteria that they were headed to SFO and it was Februrary
```{r}
sfo_feb_flights <- nycflights %>% filter(dest == 'SFO' & month==2)
length(unique(sfo_feb_flights$flight))
```
...
1.  Describe the distribution of the **arrival** delays of these flights using a 
    ...
    From the histogram it looks like more filghts arrived early than late for SFO. To confirm I filterd on arr_delay <> 0 and there are 49 flights that were early and 19 that were late.
```{r}
ggplot(sfo_feb_flights, aes(x=arr_delay)) + geom_histogram()
sfo_feb_flights %>% filter(arr_delay < 0)
sfo_feb_flights %>% filter(arr_delay > 0)
```
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(median_ad = median(arr_delay), iqr_ad=IQR(arr_delay), count=n())
```
Carriers DL and UA have the most variable arrival delays with a IQR of 22.
...
df <- nycflights %>%
  filter(dep_delay > 0) %>%
  group_by(month) %>%
  summarise(mean_dd = mean(dep_delay), median_dd = median(dep_delay), .groups = 'drop')

ggplot(df, aes(x=month, y=mean_dd)) +  geom_line()
ggplot(df, aes(x=month, y=median_dd)) + geom_line()
```
Looking at both the mean and median plots it looks like the best time to take a flight is between October and November.
The median plot is resistant to outlier while the mean is sensitive to change.
June has the highest average departure delay from NYC airport.
April has the highest median departure delay from NYC airport.
I think the median is more reliable since there could be big values moving the mean.
...
nycflights %>%
  group_by(origin) %>%
  summarise(ot_dep_rate = sum(dep_type == "on time") / n(), .groups='drop') %>%
...
Based on the ot_dep_rate airport EWR has the lowest dep_rate.
...
ggplot(nycflights, aes(x=avg_speed, y=distance, color=dep_type)) + geom_point()
nycflights %>% arrange(desc(avg_speed))
```
Funny how the fastest flight was delayed by 9 minutes but arrived 14 minutes early by going a whopping 703 MPH 
...
ggplot(df, aes(x=dep_delay, y=arr_delay, color=carrier)) + geom_point()
```
