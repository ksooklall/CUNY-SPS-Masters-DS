...
```{r}
mdq <- fastfood %>% filter(restaurant == c('Mcdonalds', 'Dairy Queen')) 
ggplot(mdq, aes(x=cal_fat, fill=restaurant)) + geom_histogram(binwidth=30) + geom_boxplot()
```

They both look right skewed with Mcdonalds having more outliers than dairy queen. Dairy queen center(median) is lower than Mcdonald but their upper IQR is above Mcdonald. Mcdonald plot is unimodal while Dairy Queens is bimodal.
...
Yes from the density plot the data looks nearly normal with the center shifted to the left
...
ggplot(data = dairy_queen, aes(sample = cal_fat)) + 
  geom_line(stat = "qq") + stat_qq(col='white') + stat_qq_line(col='red')
```
...
```{r}
ggplot(data = data.frame(sim_norm), aes(x=sim_norm)) +
        geom_blank() +
        geom_histogram(aes(y = ..density..)) +
        stat_function(fun = dnorm, args = c(mean = dqmean, sd = dqsd), col = "tomato")
```
...
More points on the qq plot for `sim_norm` are closer to the line
...
The portability plots look very similar to the simulated data, they do provide evidence that the calories are nearly normal.
...
The simulated data all look kinda normal; however the actual data has points on the right (1+) that are far away from the line
...
I will take a look at Burger King since I use to work there.
```{r}
bk <- fastfood %>% filter(restaurant == "Burger King")
bk <- bk[,c('restaurant', 'cal_fat', 'protein', 'total_carb')]
```

```{r}
qqnormsim(sample = cal_fat, data = bk)
```

```{r}
qqnormsim(sample = protein, data = bk)
```
* * *
...
```{r}
fastfood %>% filter(restaurant == restaurants[1]) %>%  ggplot(aes(sample=sodium)) + 
  geom_line(stat = "qq") + stat_qq(col='white') + stat_qq_line(col='red') + labs(title=restaurants[1])
```

```{r}
fastfood %>% filter(restaurant == restaurants[2]) %>%  ggplot(aes(sample=sodium)) + 
  geom_line(stat = "qq") + stat_qq(col='white') + stat_qq_line(col='red') + labs(title=restaurants[2])
```

```{r}
fastfood %>% filter(restaurant == restaurants[3]) %>%  ggplot(aes(sample=sodium)) + 
  geom_line(stat = "qq") + stat_qq(col='white') + stat_qq_line(col='red') + labs(title=restaurants[3])
```

```{r}
fastfood %>% filter(restaurant == restaurants[4]) %>%  ggplot(aes(sample=sodium)) + 
  geom_line(stat = "qq") + stat_qq(col='white') + stat_qq_line(col='red') + labs(title=restaurants[4])
```

```{r}
fastfood %>% filter(restaurant == restaurants[5]) %>%  ggplot(aes(sample=sodium)) + 
  geom_line(stat = "qq") + stat_qq(col='white') + stat_qq_line(col='red') + labs(title=restaurants[5])
```

```{r}
fastfood %>% filter(restaurant == restaurants[6]) %>%  ggplot(aes(sample=sodium)) + 
  geom_line(stat = "qq") + stat_qq(col='white') + stat_qq_line(col='red') + labs(title=restaurants[6])
```
   
```{r}
fastfood %>% filter(restaurant == restaurants[7]) %>%  ggplot(aes(sample=sodium)) + 
  geom_line(stat = "qq") + stat_qq(col='white') + stat_qq_line(col='red') + labs(title=restaurants[7])
```

```{r}
fastfood %>% filter(restaurant == restaurants[8]) %>%  ggplot(aes(sample=sodium)) + 
  geom_line(stat = "qq") + stat_qq(col='white') + stat_qq_line(col='red') + labs(title=restaurants[8])
```

Looks like Arbys is the closest to normal with Taco Bell and Burger King coming in close second
...
I assume the stepwise pattern is due to a value being repeated.
...
ggplot(data=bk, aes(x=total_carb)) +
        geom_blank() +
        geom_histogram(aes(y=..density..)) +
        stat_function(fun=dnorm, args=c(mean=mean(bk$total_carb), sd=sd((bk$total_carb))), col="tomato")
```

THe histogram looks normally distributed, symmetric
