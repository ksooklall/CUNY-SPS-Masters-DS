...
From my sample 63.3% think climate change affects their local community.
...
I would expect another students sampling to be similar but certainly not the same unless we have the same seed. The inherent process of sampling will produce the differences.
...
95% confidence means that we feel accurately that 51.7% and 75% of the population think climate change affects their local community.
...
Yes the true population is 62% which is between 51% and 75%. Working form home but I asked around and my neighbors interval captures the true population proportion as well.
...
Running the simulation below it looks like ~95% of the proportion will contain a point estimate that's within the true population proportion 
...
The first run of 50, 49/50 (98%) of the confidence intervals were included in the true population proportion. The second run (46/50) (92%) of the confidence intervals were included in the true population proportion. Finally I ran 100 confidence intervals and (95/100) (95%). This proportion is about the same as the confidence level.
* * *
...
It looks like the confidence level correlates to the confidence interval, 95% confidence  with produce distribution that have values within the population proportion 95% of the time.
...
49/50 (98%) of the intervals included the true population proportion, this is also inline with my hypothesis that confidence level correlates to the confidence interval
...
Using confidence level of 90% and confidence interval of 50 there are (46/50) (92%) true population proportion. As I widen the confidence level there are less true population proportion as expected.
...
Using confidence level 0.99 and intervals
```{r}
data.frame(intervals=c(10, 25, 50, 100) , population_proportion=c(9/10, 24/25, 49/50, 99/100))
```
...
Confidence level 0.95, sample size 60 confidence intervals: 50
```{r}
data.frame(bootstrap=c(10,50, 100,1000), population_proportion=c(37/50,47/50,48/50,49/50))
````

It looks like as you increase the number of bootstrap samples more population_proportion are within the true population_proportion
* * *