...
```{r}
global_monitor %>% sample_n(50) %>% count(scientist_work) %>% mutate(p=n/sum(n))
```
...
The sample population varies by 2% from the true population distribution.
...
Depending on the seed and how sample_n function works I don't believe my sampling will match another student. I ran the same line of code multiple times are got different answers before seeding.
...
samp2 <- global_monitor %>% sample_n(s2) %>% count(scientist_work) %>% mutate(pop=n/sum(n))
samp3 <- global_monitor %>% sample_n(s3) %>% count(scientist_work) %>% mutate(pop=n/sum(n))
samp4 <- global_monitor %>% sample_n(s4) %>% count(scientist_work) %>% mutate(pop=n/sum(n))
```

As the sample number goes up the sample distribution gets closer to the true population distribution
...
ggplot(data = sample_props50, aes(x = p_hat)) +
  ...
  stat_function(fun=dnorm, args=c(mean=mean(sample_props50$p_hat), sd=sd(sample_props50$p_hat)), col="tomato") + 
  labs(
...
There are 15k elements in `sample_props50`. The distribution is very close to normal with a center at 0.2 and std of 0.05.
...
Each observation represent n persons belief on the benefit of science
...
As the sample size goes up the standard error goes down
...
```{r}
samp15 <- global_monitor %>% sample_n(15) %>% count(scientist_work) %>% mutate(pop=n/sum(n))
```

From my sample it looks like 60% of the population think scientist_work has Benefit their lives
...
ggplot(data=sample_props15, aes(x = p_hat)) + geom_histogram(binwidth = 0.02, aes(y=..density..)) +
  stat_function(fun=dnorm, args=c(mean=mean(sample_props15$p_hat), sd=sd(sample_props15$p_hat)), col="tomato") + 
  labs(
    x = "p_hat (Benefits)",
    title = "Sampling distribution of p_hat",
...
The distribution is left skewed with a mean of 0.8 which is suprisingly inline with the population mean.
...
ggplot(data=sample_props150, aes(x = p_hat)) + geom_histogram(binwidth = 0.02, aes(y=..density..)) +
  stat_function(fun=dnorm, args=c(mean=mean(sample_props15$p_hat), sd=sd(sample_props15$p_hat)), col="tomato") + 
  labs(
    x = "p_hat (Benefits)",
    title = "Sampling distribution of p_hat",
...
The distribution is more normal with a mean of 0.8 and looks more continous.
...
The spread of (2) is very grainular while (3) is more continous. I would assume larger spread is better since you are more like to be close to the true mean/std.
However samples from a population should be no larger than 10% of the population. When the sample exceeds 10% of the population size, sampling tends to overestimate the sampling error slightly.
* * *
