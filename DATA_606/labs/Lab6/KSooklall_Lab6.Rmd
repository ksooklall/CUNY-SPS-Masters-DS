...
```{r}
df <- yrbss %>% drop_na(text_while_driving_30d)
table(df$text_while_driving_30d)
```
...
```{r}
df %>% filter(helmet_12m == 'never') %>% mutate(text_ind = ifelse(text_while_driving_30d == "30", "yes", "no")) %>% count(text_ind) %>% mutate(p=n/sum(n))
```
...
pyes = 1 - pno
n = 6503
c(pyes - 1.96 * sqrt(pyes * (1-pyes)/n), pyes + 1.96 * sqrt(pyes * (1-pyes)/n))
```

Between 6.5 and 7.7% of individuals texted while driving each day for the past 30 days without wearing a helmet with 95% confidence. 
...
physically_active_7d vs high school seniors
```{r}
df %>% filter(grade == '12') %>% drop_na() %>%
  mutate(active = ifelse(physically_active_7d > 0, 'yes', 'no')) %>%
...
According to the sample between 79-82% of high school seniors are active for at least 1 hour in a 7 day window.
...
The margin of error is 0 when the population proportion is singular which is expected. The margin of error grows as the population proportion get's larger and peaks at 0.5 then drops back to 0. 
...
For n=300 and p=0.1 (q=0.9) the center is at p and the shape is unimodal normal.
...
As p increases the center of the distribution moves accordingly (from left to right)
the spread also widens a bit and the distribution is still normally shaped.

#### Exercise 8
Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?

As $n$ increased the distribution narrows which makes sense since a larger $n$ will cause $\hat{p}$ to converge to $p$.
* * *
...
$H_o$ = sleep 10+ hours per day has no affect on someone likely to strength train every day of the week
$H_a$ = sleep 10+ hours per day makes one likely to strength train every day of the week

```{r}
df %>% filter(school_night_hours_sleep == '10+') %>% drop_na() %>%
  mutate(active = ifelse(strength_training_7d == 7, 'yes', 'no')) %>%
  specify(response = active, success = "yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
  get_ci(level = 0.95)
...
To keep the margin of error below 1% n
```{r}
n = seq(100, 1080, 10)
p = seq(0.01, 0.99, 0.01)
m = 2 * sqrt(p * (1-p) / n)
mdf <- data.frame(cbind(n,p,m))

ggplot(data = mdf, aes(x = n, y = m)) + 
  geom_line() +
  labs(x = "Population Sample", y = "Margin of Error")

scatterplot3d(mdf, grid=T)
``` 

From the plot it looks like an n of almost 1000 would be needed to keep the margin of error bounded by 1%.
