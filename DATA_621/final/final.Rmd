...
TODO: 
*******************
Groupby fullVistorId agg sum tranRev, and other numeric values            DONE
look at month day and year                                                DONE
second model GLM
final section comparing GLM and RF
Final project presentation slide shows
...
```{r}
features <- c('transactionRevenue', 'channelGrouping', 'browser', 'operatingSystem', 'isMobile', 'deviceCategory', 'continent', 'subContinent', 'country', 'region', 'metro', 'city', 'networkDomain', 'hits', 'pageviews', 'source', 'medium')

df_rec <- recipe(transactionRevenue ~ ., data=df_train %>% select(features)) %>%
...
Our model has a RMSE of 1.08 on the test set which is a little better than on the training set of `rmse_val`.
...
y_pred <- predict(model, df_test %>% select(features[2:17]))
rf_df <- data.frame(y_pred=y_pred$.pred, y_true=df_test %>% select(transactionRevenue))
rmse <- sqrt(mean((rf_df$transactionRevenue - rf_df$y_pred)^2))
p1 <- rf_df %>% 
  ggplot(aes(x=y_pred, y=transactionRevenue)) + 
    geom_point() +
...
```{r}
glm1 <- glm(transactionRevenue ~ ., data=df_train %>% select(features), family='gaussian')
y_pred <- predict(glm1, newdata = df_test %>% select(features[2:17]))
glm_df <- data.frame(y_pred=y_pred, y_true=df_test %>% select(transactionRevenue))
```

```{r}
rmse <- sqrt(mean((glm_df$transactionRevenue - glm_df$y_pred)^2))
p2 <- glm_df %>% 
  ggplot(aes(x=y_pred, y=transactionRevenue)) + 
    geom_point() +
...
The random forest model is more narrow than the GLM model which explains why its RMSE is lower.
...
```{r}
test <- read.csv(paste0(path, 'test_clean.csv')) %>% select(features[2:17])
```
