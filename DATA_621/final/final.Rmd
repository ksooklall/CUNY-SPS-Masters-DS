...
gg_miss_var(df %>% filter(transactionRevenue > 0.01), show_pct = TRUE)
```
...
df <- df %>% 
  select(-c(all_of(uniq_cols), all_of(drop_cols)))
```
...
#df %>% filter(transactionRevenue > 0) %>% ggplot(aes(x=transactionRevenue, fill=deviceCategory)) + geom_histogram(position='identity', alpha=0.75, bins=50) + scale_x_log(labels=scales::dollar_format())
grid.arrange(p1, p2)
```
...
I add 0.01 to the transactionRevenue to avoid -inf when taking the log

```{r}
set.seed(42)
df_split <- df %>% 
  filter(transactionRevenue > 0) %>%
  mutate(trevenue = log(transactionRevenue)) %>%
...
set.seed(43)
df_folds <- bootstraps(df_train, strata = trevenue, times=10)
```
...
df_rec <- recipe(trevenue ~ channelGrouping + browser + operatingSystem + 
                   ...
                   city + pageviews, hits, data=df_train) %>%
  step_novel(operatingSystem, country, city, browser) %>%
...
The metric used in this competation is RMSE
```{r}
