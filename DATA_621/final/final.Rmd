...
factor_cols <- c('visitNumber', 'hits', 'pageviews', 'bounces', 'newVisits')
...
```{r}
# sapply(df_train %>% select(features), function(x) sum(is.na(x)))
glm1 <- glm(transactionRevenue ~ ., data=smp %>% select(features), family='gaussian')
...
```{r}
rmse <- sqrt(mean((glm_df$y_true - glm_df$y_pred)^2))
p2 <- glm_df %>% 
  ggplot(aes(x=y_pred, y=y_true)) + 
    geom_point() +
