...
For this analysis we will concentrate on the responses to the mood disorders questions as mood swings seems to be a strong predictor for suicide attempts.
...
Next, we'll visualize these components to better understand this analysis. While plotting a PCA we refer to a scatter plot of the first two principal components PC1 and PC2. These plots reveal the features of data such as non-linearity and departure from normality. PC1 and PC2 are evaluated for each sample vector and plotted. 
...
```{r}
plot.pca.out <- plot(pca.out, type="l")
plot.pca.out
```  
...
We will tune the model with the training set to find the best combination with lower RMSE error. 
...
After this procedure, we took our top model's  hyperparameter settings and used gbm function setting parameters the following way:
...
modelGBM=gbm(as.character(suicide)~., data=data.frame(df_train)
             ,n.trees=1000,distribution='adaboost',interaction.depth=3,shrinkage=0.05, n.minobsinnode = 15)

print(modelGBM)
```


The summary function gave us a variable importance plot. `adhd_total`and `md_total` are the most important variables in the model.
...
It appears that Boosting has a correct classification rate of 69% and a specificity rate is 81%. We'll use this as a benchmark as we move forward to an SVM method.
...
Our base model consists of 87 support vectors where 53 are assign to label 0 (no suicide) and 34 to label 1 (suicide)
...
Now with the best model we can test it against the train and test set
...
On the training set the model made few mistakes. It is most accurate at predicting someone will not commit suicide with 83 true negative The largest mistake is 7 false positives, the model predicted that those individuals would commit suicide when they didn't. There is also one false negative. 

On the testing set the same relation applies but the errors are a bit more exaggerated. The most accurate metric is still the true negatives at 25. There is an equal amount of false positives with the training set. However there are 4 false negatives, a situation where the model predicts those individual won't commit suicide when they did.
