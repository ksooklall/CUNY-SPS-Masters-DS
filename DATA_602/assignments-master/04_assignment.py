...
def exercise08():
    ...
    df_zip_95670 = df[df['zip'].eq(95670)]
    df_zip_not_95610 = df[~df['zip'].eq(95610)]
...
def exercise13(n):
    ...
    df = pd.DataFrame({'date': pd.date_range(start='2010-01-01',
                                             end='2015-02-01',
                                             periods=1000)})
    df['s'] = np.random.randint(0, n, size=(df.shape[0], 1))
...
def exercise14(words):
    ...
    df = pd.DataFrame({'words': words}).assign(lens=lambda x: x['words'].str.len)
