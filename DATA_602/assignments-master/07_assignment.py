...
iris = ds.load_iris()
boston = ds.load_boston(return_X_y=True)
df = pd.DataFrame(data=iris['data'], columns=iris['feature_names']).assign(target=iris['target'])
...
def exercise02(new_observations):
    ...
    knn.fit(df.drop(['target'], axis=1), df['target'])

    y_pred = knn.predict(new_observations)
    maps = {i: v for i, v in enumerate(iris['target_names'])}
    iris_predictions = [maps[i] for i in y_pred]
    # ------ Place code above here /\ /\ /\ ------
...
def exercise03(neighbors,split):
    ...
    # ------ Place code below here \/ \/ \/ ------
    X_train, X_test, y_train, y_test = tts(df.drop(['target'], axis=1), df['target'], test_size=split)
    knn = KNN(n_neighbors=5)
    knn.fit(X_train, y_train)

    y_pred = knn.predict(X_test)
    knn_score = sum(y_test == y_pred)/len(y_pred)
    # ------ Place code above here /\ /\ /\ ------
...
def exercise04():
    ...
    for i in range(1, 30):
        ...
        knn.fit(X_train, y_train)
        y_pred = knn.predict(X_test)
        scores.append(sum(y_test == y_pred)/len(y_pred))

    pd.DataFrame({'neighbors': [i for i in range(1, 30)], 'scores': scores}).plot(kind='bar')
